...
void calculator::run() {
    ...
    try {
            ...
            if (menuNum == "0") {                                                   //Exit the program
                ...
                for (int i = 0; i < token.size(); i++) {
                    std::string tmp = token[i];
                    std::string s;
                    if (tmp != "(" && tmp != ")" && tmp != "+" && tmp != "-" && tmp != "*" && tmp != "/" && tmp != "^"&& verifyVariable(tmp) ){
                        std::cout << tmp << " : ";
                        getline(std::cin, s); if (verifyVariable(s)) throw s;
                        token[i] = s;
                        for (int j = i+1; j < token.size(); j++) {
                            if (token[j] == tmp) token[j] = s;
                        }
                    }
                }
                covertToPostFix(token, postFix);
                ...
                std::string input;
                std::cout << "Enroll Formula by using operator and variable A~Z(exit 0) " << std::endl;
                std::cout << "Formula : ";
                std::getline(std::cin, input);
                if (input == "0") {                                                   //Exit the program
                    std::cout << "Exit the program!";
